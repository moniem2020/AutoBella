'use client';

import React from 'react';
import Link from 'next/link';

interface SocialBubblesProps {
    className?: string;
    size?: 'sm' | 'md' | 'lg';
}

const socialLinks = [
    {
        name: 'Instagram',
        url: 'https://www.instagram.com/autobella_carwash/?igsh=MWlzMTF0ZXJydXczcQ%3D%3D#',
        icon: (
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full p-2.5">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
            </svg>
        ),
        gradient: 'from-[#833AB4] via-[#FD1D1D] to-[#F77737]',
        hoverScale: 'hover:scale-110',
    },
    {
        name: 'TikTok',
        url: 'https://www.tiktok.com/@autobella.me',
        icon: (
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full p-2.5">
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
            </svg>
        ),
        gradient: 'from-[#000000] to-[#25F4EE]',
        hoverScale: 'hover:scale-110',
    },
    {
        name: 'Facebook',
        url: 'https://www.facebook.com/autobella.eg',
        icon: (
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full p-2.5">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
            </svg>
        ),
        gradient: 'from-[#1877F2] to-[#3b5998]',
        hoverScale: 'hover:scale-110',
    },
];

const SocialBubbles: React.FC<SocialBubblesProps> = ({ className = '', size = 'md' }) => {
    const sizeClasses = {
        sm: 'w-10 h-10',
        md: 'w-12 h-12',
        lg: 'w-14 h-14',
    };

    return (
        <div className={`flex items-center gap-3 ${className}`}>
            {/* Optional label */}
            <span className="text-white/60 text-sm font-medium hidden sm:block">Follow Us</span>

            {/* Bubbles container */}
            <div className="flex items-center gap-2">
                {socialLinks.map((social, index) => (
                    <Link
                        key={social.name}
                        href={social.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`
                            ${sizeClasses[size]}
                            relative
                            rounded-full
                            bg-gradient-to-br ${social.gradient}
                            text-white
                            shadow-lg
                            transition-all duration-300 ease-out
                            ${social.hoverScale}
                            hover:shadow-xl hover:shadow-[#C9A961]/30
                            group
                            animate-bounce-subtle
                        `}
                        style={{
                            animationDelay: `${index * 0.15}s`,
                        }}
                        aria-label={`Follow us on ${social.name}`}
                    >
                        {/* Bubble shine effect */}
                        <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-white/30 via-transparent to-transparent opacity-60" />

                        {/* Bubble border glow */}
                        <div className="absolute inset-0 rounded-full border border-white/20 group-hover:border-white/40 transition-colors" />

                        {/* Icon */}
                        <div className="relative z-10 w-full h-full">
                            {social.icon}
                        </div>

                        {/* Floating bubble effect on hover */}
                        <div className="absolute -inset-1 rounded-full bg-white/10 opacity-0 group-hover:opacity-100 blur-sm transition-opacity" />
                    </Link>
                ))}
            </div>
        </div>
    );
};

// Add custom animation to global styles
const BubbleStyles = () => (
    <style jsx global>{`
        @keyframes bounce-subtle {
            0%, 100% {
                transform: translateY(0);
            }
            50% {
                transform: translateY(-3px);
            }
        }
        .animate-bounce-subtle {
            animation: bounce-subtle 3s ease-in-out infinite;
        }
    `}</style>
);

// Wrapper component that includes styles
const SocialBubblesWithStyles: React.FC<SocialBubblesProps> = (props) => (
    <>
        <BubbleStyles />
        <SocialBubbles {...props} />
    </>
);

export default SocialBubblesWithStyles;
